...
actor EmbeddingGemmaModel {
  ...
  /// Output: 768-dimensional embedding vector (unnormalized)
  func inference(tokenIds: [Int]) throws -> [Float]? {
    ...
    do {
      // Convert token IDs to MLMultiArray input
      let inputArray = try MLMultiArray(shape: [NSNumber(value: tokenIds.count)], dataType: .int32)
      for (i, tokenId) in tokenIds.enumerated() {
        inputArray[i] = NSNumber(value: Int32(tokenId))
      }

      // Prepare input: EmbeddingGemma expects {"input_ids": inputArray}
      let input = try EmbeddingGemmaModelInput(input_ids: inputArray)
      let output = try model.prediction(input: input) as! EmbeddingGemmaModelOutput

      // Extract embedding vector (768-dim)
...
struct EmbeddingGemmaModelInput: MLFeatureProvider {
  let input_ids: MLMultiArray

  var featureNames: Set<String> {
    return Set(arrayLiteral: "input_ids")
  }
